...
-- what is accessible to you at the top level
-- (==) :: Eq a => a -> a -> Bool
...
-- eqInt :: Eq Int
-- eqInt = MkEq <primitive> not

-- (==) :: Eq a -> a -> a -> Bool
-- (==) (MkEq f _) a b = f a b

-- negate (MkEq _ n) bool = n bool


-- fac = \n -> if n <primitive eq> 0 then 1 else n * fac (n-1)
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  (=<<) apb (P pa) = P (\input -> onResult (pa input) (\input2 a0 -> parse (apb a0) input2))
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser b
  (<*>) (P pab) (P pa) = 
    P (\input -> onResult (pab input) (\input2 fab -> fab <$> pa input2))
...
-- True
list1 ::
  Parser a -- Monad m =>  m a -> m (List a)
  -> Parser (List a)
list1 pa = do 
  li <- list pa 
  case li of 
    Nil -> constantParser UnexpectedEof
    _ -> pure li 
...
firstNameParser ::
  Parser Chars
firstNameParser = do -- (:.) <$> upper <*> list lower
  firstletter <- upper
  rest <- list lower 
...
surnameParser = do
  firstletter <- upper
  rest1 <- thisMany 5 lower 
  rest2 <- list lower 
  pure (firstletter :. rest1 ++ rest2)
...
personParser = do 
  ...
  spaces1
  lastname <- surnameParser
  spaces1
  issmoke <- smokerParser
  spaces1
  num <- phoneParser
  pure (Person age name lastname issmoke num)
