...
oneof ::
  Chars
  -> Parser Char
oneof string = satisfy (\char -> elem char string)
...
noneof ::
  Chars
  -> Parser Char
noneof string = satisfy (\char -> notElem char string)
...
hex ::
  Parser Char
hex = do
  hexread <- replicateA 4 (satisfy isHexDigit)
  case readHex hexread of 
    Full dig -> (pure . chr) dig
    Empty -> constantParser (UnexpectedString hexread)
...
eof ::
  Parser ()
eof = P (\input -> if isEmpty input then Result input () else ExpectedEof input)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll li = satisfy (and . sequence li)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny li = satisfy (or . sequence li)
